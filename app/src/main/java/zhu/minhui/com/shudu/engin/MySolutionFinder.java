...
public class MySolutionFinder implements ISoduSolutionFinder {
    ...
    public int findSolution(char[] charArray) {
        initData(charArray);
        //Log.d("");
        Log.d(TAG, "begin to findSolution");
        if (!findSoduResult()) {
            Log.d(TAG, "not find result findIndex" + findIndex);
            return NONE_RESULT;
        }
        hasFoundResult = true;
        Log.d(TAG, " find result " + "   findIndex" + findIndex);
        for (int m = 80; m >= 0; m--) {
            int i = m / 9;
            int k = m % 9;
            ...
            Log.d(TAG, "find double num: i:  " + i + "   k:   " + k);
            ...
            while (iterator.hasNext()) {
                ...
                if (findSoduResult()) {
                    Log.d(TAG, " success  find double num: i:  " + i + "   k:   " + k + "   findIndex" + findIndex);
                    return MULTI_RESULT;
                }
            }
            soduNodes[i][k].value = 0;
